...
export default function DashboardPage() {
  ...
  return (
    <div className="bg-[#f0fdfa] text-slate-900 min-h-screen font-sans selection:bg-[#ffc174]/30 overflow-x-hidden">
      <ActionModal
      ...
      {/* Navigation */}
      <nav className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 md:px-10 py-4 bg-white/60 backdrop-blur-xl border-b border-teal-100 shadow-lg shadow-[#ffc174]/10">
        ...
        <section>
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <h2 className="text-3xl font-semibold text-slate-900 flex items-center gap-3">
              ...
              Your Itineraries
              <span className="text-sm font-normal text-slate-600 bg-white/5 px-3 py-1 rounded-full ml-2 border border-teal-100">{trips.length} Saved</span>
            </h2>
            <div className="relative w-full md:w-72">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
              <input type="text" placeholder="Search trips..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white/60 backdrop-blur-md border border-teal-100 rounded-xl py-2.5 pl-10 pr-4 text-sm text-slate-900 placeholder:text-slate-600/50 focus:outline-none focus:border-[#ffc174]/50 transition-colors shadow-inner" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {loading ? (
              <div className="col-span-full py-20 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-xl text-slate-600 font-medium">Fetching your planned journeys...</p>
              </div>
            ) : trips.length === 0 ? (
               <div className="col-span-full py-16 text-center text-slate-600 font-medium text-xl bg-white/40 rounded-3xl border border-dashed border-white/20 backdrop-blur-md">
              ...
              trips.filter(t => t.destination.toLowerCase().includes(searchTerm.toLowerCase())).map((trip) => (
                <div key={trip.id} className="group relative backdrop-blur-xl bg-white/40 border border-teal-100 rounded-2xl p-1 hover:border-[#ffc174]/40 hover:bg-white/60 hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#ffc174]/10 transition-all duration-500 overflow-hidden">
                  <div className="h-48 relative overflow-hidden bg-white rounded-t-2xl">
                  ...
                  <div className="p-6 relative z-20">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="text-center w-1/3">
                        <p className="text-slate-600 text-xs font-semibold uppercase tracking-wider mb-1 truncate">{trip.destination}</p>
                    ...
                    <div className="space-y-3 mb-8">
                      ...
                      <div className="flex items-center gap-3 text-slate-900 font-semibold bg-white/5 p-2 rounded-lg border border-teal-50">
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-teal-50">
                      <Link href={`/itinerary/${trip.id}`}>
                        <button className="flex items-center gap-2 text-slate-600 hover:text-[#ffc174] transition-colors cursor-pointer">
                      ...
                      <button onClick={() => handleDownload(trip.id)} className="flex items-center gap-2 text-slate-600 hover:text-[#0f766e] transition-colors cursor-pointer">
            ...
            <Link href="/plan" className="block focus:outline-none">
              <div className="h-full min-h-[420px] backdrop-blur-xl bg-white/40 border-2 border-dashed border-teal-100 rounded-2xl flex flex-col items-center justify-center p-12 text-center group cursor-pointer hover:border-[#ffc174]/50 hover:bg-white/80 transition-all duration-500 shadow-lg">
                ...
                <h3 className="text-2xl font-semibold text-slate-900 mb-2 tracking-wide">Forge a New Journey</h3>
                <p className="text-base text-slate-600 max-w-[200px] mb-8 group-hover:text-[#ffc174] transition-colors">
                    The world is wide. Let our AI map your path.
                </p>
                <div className="w-12 h-12 rounded-full bg-white/5 border border-teal-100 flex items-center justify-center text-[#ffc174] group-hover:bg-[#ffc174] group-hover:text-[#2a1700] group-hover:shadow-[0_0_20px_rgba(245,158,11,0.5)] transition-all duration-300">
