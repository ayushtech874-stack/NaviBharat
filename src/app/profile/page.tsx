...
export default function ProfilePage() {
  ...
  return (
    <div className="bg-[#020617] text-[#dce1fb] min-h-screen font-sans overflow-x-hidden relative" style={{ background: 'radial-gradient(circle at 0% 0%, rgba(245, 158, 11, 0.08) 0%, transparent 35%), radial-gradient(circle at 100% 100%, rgba(20, 184, 166, 0.08) 0%, transparent 35%), #020617' }}>
      ...
      {/* TopNavBar */}
      <nav className="fixed top-0 w-full z-50 bg-[#191f31]/60 backdrop-blur-xl border-b border-white/10 shadow-lg shadow-[#f59e0b]/5">
      ...
      <main className="pt-32 pb-40 px-6 md:px-10 max-w-[1280px] mx-auto">
        {/* Breadcrumb */}
        <nav className="mb-8 flex items-center gap-2 text-sm font-semibold text-[#d8c3ad]/60">
          <Link className="hover:text-[#ffc174] transition-colors" href="/dashboard">Dashboard</Link>
          <ChevronRight size={16} />
          <span className="text-[#d8c3ad]">My Profile</span>
        </nav>

        {/* Hero Card */}
        <section className="bg-[#191f31]/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 mb-12 relative overflow-hidden shadow-[0_0_40px_-10px_rgba(245,158,11,0.3)]">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-12">
            ...
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              ...
              <div className="h-32 w-32 md:h-44 md:w-44 rounded-full overflow-hidden border-4 border-white/10 ring-4 ring-[#ffc174]/20 bg-gradient-to-br from-[#f59e0b] to-[#d97706] flex items-center justify-center text-4xl font-bold text-white shadow-xl">
                 {profilePicUrl ? (
            ...
            <div className="text-center md:text-left flex-1">
              <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2">
                <h1 className="text-3xl md:text-4xl font-bold text-[#dce1fb]">{name || "Explorer"}</h1>
                <span className="w-fit mx-auto md:mx-0 px-3 py-1 bg-[#4fdbc8]/20 text-[#4fdbc8] text-xs font-semibold rounded-full border border-[#4fdbc8]/30">Joined {joinedYear}</span>
              </div>
              <p className="text-lg text-[#d8c3ad] mb-1">{user?.email}</p>
              <p className="text-sm font-semibold text-[#ffc174] mb-4">{occupation || "No occupation added"}</p>
              <p className="italic text-[#d8c3ad]/80 text-base max-w-lg">
        ...
        <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-[#191f31]/60 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-[0_0_40px_-10px_rgba(20,184,166,0.3)]">
            <div className="flex items-center gap-3 mb-8">
              <svg className="text-[#4fdbc8] w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
              <h2 className="text-2xl font-bold text-[#dce1fb]">Personal Information</h2>
            </div>
            
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-[#d8c3ad] ml-1">Full Name</label>
                <input required onChange={(e) => setName(e.target.value)} value={name} className="w-full bg-[#070d1f] border border-white/10 rounded-xl px-4 py-3 text-[#dce1fb] focus:ring-2 focus:ring-[#ffc174] focus:border-transparent outline-none transition-all shadow-[0_0_15px_rgba(245,158,11,0)] focus:shadow-[0_0_15px_rgba(245,158,11,0.1)]" type="text" placeholder="Your full name" />
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-[#d8c3ad] ml-1">Phone Number</label>
                  <input onChange={(e) => setPhone(e.target.value)} value={phone} className="w-full bg-[#070d1f] border border-white/10 rounded-xl px-4 py-3 text-[#dce1fb] focus:ring-2 focus:ring-[#ffc174] focus:border-transparent outline-none transition-all shadow-[0_0_15px_rgba(245,158,11,0)] focus:shadow-[0_0_15px_rgba(245,158,11,0.1)]" type="tel" placeholder="+91 98765 43210" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-[#d8c3ad] ml-1">Gender</label>
                  <select onChange={(e) => setGender(e.target.value)} value={gender} className="w-full bg-[#070d1f] border border-white/10 rounded-xl px-4 py-3 text-[#dce1fb] focus:ring-2 focus:ring-[#ffc174] focus:border-transparent outline-none transition-all shadow-[0_0_15px_rgba(245,158,11,0)] focus:shadow-[0_0_15px_rgba(245,158,11,0.1)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-[#d8c3ad] ml-1">Age</label>
                  <input onChange={(e) => setAge(e.target.value)} value={age} className="w-full bg-[#070d1f] border border-white/10 rounded-xl px-4 py-3 text-[#dce1fb] focus:ring-2 focus:ring-[#ffc174] focus:border-transparent outline-none transition-all shadow-[0_0_15px_rgba(245,158,11,0)] focus:shadow-[0_0_15px_rgba(245,158,11,0.1)]" type="number" placeholder="28" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-[#d8c3ad] ml-1">Date of Birth</label>
                  <input onChange={(e) => setDateOfBirth(e.target.value)} value={dateOfBirth} className="w-full bg-[#070d1f] border border-white/10 rounded-xl px-4 py-3 text-[#dce1fb] focus:ring-2 focus:ring-[#ffc174] focus:border-transparent outline-none transition-all shadow-[0_0_15px_rgba(245,158,11,0)] focus:shadow-[0_0_15px_rgba(245,158,11,0.1)]" type="date" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#191f31]/60 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-[0_0_40px_-10px_rgba(245,158,11,0.3)] h-full">
            <div className="flex items-center gap-3 mb-8">
      ...
      {/* Bottom Action Bar */}
      <footer className="fixed bottom-0 left-0 w-full z-40 bg-[#070d1f]/80 backdrop-blur-md border-t border-white/5">
        <div className="flex justify-between items-center py-4 md:py-6 px-6 md:px-10 w-full max-w-[1280px] mx-auto gap-4">
          <div className="flex items-center gap-2">
            <svg className="text-[#d8c3ad]/40 w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>
            <p className="text-sm font-semibold text-[#d8c3ad]">Member Since {joinedYear}</p>
          </div>
