...
export default function PlanTripPage() {
  ...
  if (isGenerating) {
    return (
      <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center text-white p-4 relative">
        {destInfo?.image ? (
        ...
        <div className="z-10 flex flex-col items-center max-w-md w-full text-center space-y-8">
          ...
          <div>
            <h2 className="text-3xl font-bold mb-2 tracking-tight text-[#dce1fb]">Crafting Your Dream Journey...</h2>
            <p className="text-[#d8c3ad] text-lg flex items-center justify-center gap-2">
  ...
  return (
    <div className="bg-[#020617] text-[#dce1fb] font-sans selection:bg-[#ffc174] selection:text-[#472a00] min-h-screen relative">
      ...
      <main className="relative z-10 pt-32 pb-24 px-6 md:px-10 max-w-[1280px] mx-auto">
        ...
        <form onSubmit={handleGenerate} className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          <div className="col-span-12 lg:col-span-8">
            <div className="bg-[#0c1324]/60 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 border-t-4 border-t-[#4fdbc8]/50 shadow-2xl">
              <h2 className="text-2xl font-bold mb-8 text-white">Expedition Logistics</h2>
              <div className="space-y-8">
                {/* Origin & Destination */}
                <div className="flex flex-col md:flex-row gap-6 items-center">
                  <div className="w-full relative">
                    <label className="block text-xs font-semibold text-[#d8c3ad] mb-2 uppercase tracking-wider">Origin</label>
                    <div className="relative">
                      ...
                      <input required type="text" value={source} onChange={e => setSource(e.target.value)} placeholder="e.g. New Delhi, India" className="w-full bg-[#020617] border border-white/10 rounded-xl py-4 pl-10 pr-4 text-[#dce1fb] focus:border-[#ffc174] focus:ring-0 transition-all outline-none shadow-inner" />
                    </div>
                  </div>
                  <div className="text-[#d8c3ad] pt-6 hidden md:block opacity-50">
                  ...
                  <div className="w-full relative">
                    <label className="block text-xs font-semibold text-[#d8c3ad] mb-2 uppercase tracking-wider">Destination</label>
                    <div className="relative">
                      ...
                      <input required type="text" value={destination} onChange={e => setDestination(e.target.value)} placeholder="Where to?" className="w-full bg-[#020617] border border-white/10 rounded-xl py-4 pl-10 pr-4 text-[#dce1fb] focus:border-[#4fdbc8] focus:ring-0 transition-all outline-none shadow-inner" />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="flex items-center justify-between p-4 bg-[#191f31] rounded-xl border border-white/5">
                    <span className="font-semibold text-sm text-[#dce1fb]">Travel Party</span>
                    <div className="flex items-center gap-4">
                      <button type="button" onClick={() => setTravelers(Math.max(1, travelers - 1))} className="w-8 h-8 flex items-center justify-center rounded-lg border border-white/10 hover:bg-white/5 text-[#ffc174]">-</button>
                      <span className="font-bold text-lg min-w-[20px] text-center">{travelers.toString().padStart(2, '0')}</span>
                      <button type="button" onClick={() => setTravelers(travelers + 1)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-white/10 hover:bg-white/5 text-[#ffc174]">+</button>
                    </div>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-[#191f31] rounded-xl border border-white/5">
                    <span className="font-semibold text-sm text-[#dce1fb]">Projected Cost</span>
                    <span className="bg-[#4fdbc8]/20 text-[#4fdbc8] px-4 py-1 rounded-full font-bold">₹{totalGroupBudget.toLocaleString('en-IN')}</span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <label className="block text-xs font-semibold text-[#d8c3ad] uppercase tracking-wider">Per Person Budget</label>
                    <div className="flex items-center gap-2">
                      <span className="text-[#ffc174]">₹</span>
                      <input type="number" value={perHeadBudget} onChange={e => setBudget([parseInt(e.target.value) || 0])} className="bg-transparent border-b border-[#ffc174] text-right font-bold text-2xl w-32 focus:ring-0 py-0 px-1 outline-none text-[#dce1fb]" />
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-semibold text-[#d8c3ad] mb-2 uppercase tracking-wider">Trip Duration</label>
                    <div className="flex flex-col gap-2">
                      ...
                      }} className="w-full bg-[#020617] border border-white/10 rounded-xl py-4 px-4 text-[#dce1fb] focus:border-[#ffc174] focus:ring-0 transition-all appearance-none outline-none">
                      ...
                      {daysMode === "custom" && (
                        <input type="number" placeholder="Enter number of days" value={customDays} onChange={e => setCustomDays(e.target.value)} className="w-full bg-[#020617] border border-white/10 rounded-xl py-4 px-4 text-[#dce1fb] placeholder:text-white/20 focus:border-[#ffc174] focus:ring-0 transition-all outline-none" min="1" max="90" />
                      )}
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[#d8c3ad] mb-2 uppercase tracking-wider">Travel Style</label>
                    <select value={style} onChange={e => setStyle(e.target.value)} className="w-full bg-[#020617] border border-white/10 rounded-xl py-4 px-4 text-[#dce1fb] focus:border-[#ffc174] focus:ring-0 transition-all appearance-none outline-none">
                ...
                <div>
                  <div className="flex justify-between items-end mb-4">
                     <label className="block text-xs font-semibold text-[#d8c3ad] uppercase tracking-wider">The Vibe</label>
                     <span className="text-xs text-[#d8c3ad]">{preferences.length}/4 selected</span>
                  </div>
                    ...
                    {preferencesOptions.map(pref => {
                       ...
                       return (
                          <button 
                             ...
                             className={`px-6 py-2 rounded-full font-bold transition-all ${isActive ? 'bg-gradient-to-r from-[#f59e0b] to-[#d97706] text-white shadow-lg shadow-[#f59e0b]/20' : 'border border-white/10 hover:border-[#4fdbc8] hover:text-[#4fdbc8] text-[#dce1fb]'}`}
                          >
                             {pref}
                          </button>
                       )
          ...
          <div className="col-span-12 lg:col-span-4 sticky top-28">
            <div className="bg-[#0c1324]/60 backdrop-blur-2xl rounded-3xl overflow-hidden border border-white/10 h-[700px] flex flex-col shadow-2xl relative">
              <div className="flex-grow relative bg-[#020617] overflow-hidden">
                ...
                <div className="absolute inset-0 bg-[#0c1324]/10 pointer-events-none"></div>
                ...
                {destination && !destCoords && (
                   <div className="absolute top-1/3 left-1/2 -translate-x-1/2 flex flex-col items-center pointer-events-none z-10">
                      ...
                      <span className="mt-2 bg-[#191f31] px-3 py-1 rounded-lg text-xs font-semibold border border-white/10 text-white">{destination}</span>
                   </div>
                )}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 pointer-events-none" style={{ background: 'linear-gradient(to top, rgba(12, 19, 36, 1) 0%, rgba(12, 19, 36, 0.7) 50%, rgba(12, 19, 36, 0) 100%)' }}>
                  {destInfo ? (
                     <div className="flex gap-4 items-start pointer-events-auto bg-[#0c1324]/50 backdrop-blur-md p-4 rounded-2xl border border-white/5 shadow-xl">
                        {destInfo.image && (
                           <img src={destInfo.image} className="w-20 h-20 rounded-xl object-cover border border-white/20 shadow-lg" alt={destination} />
                        )}
                        <div className="flex-1">
                           <h3 className="font-bold text-lg text-white mb-1 line-clamp-1">{destination}</h3>
                           <p className="text-xs font-semibold text-[#d8c3ad] line-clamp-3 leading-relaxed">
                              {destInfo.extract}
                           </p>
                           <span className="text-[10px] text-white/40 mt-2 block uppercase tracking-widest">Data via Wikipedia</span>
                        </div>
                     </div>
                  ) : destination ? (
                     <div className="flex gap-4 items-center justify-center p-4">
                         <Loader2 className="animate-spin text-[#4fdbc8] w-6 h-6" />
                         <span className="text-[#d8c3ad] text-sm font-semibold">Gathering intel...</span>
                     </div>
                  ) : (
                     <div className="text-center p-4">
                         <span className="text-white/40 text-xs uppercase tracking-widest font-semibold border border-white/10 px-4 py-2 rounded-full">Awaiting Destination</span>
                     </div>
                  )}
                </div>
              </div>
