...
export default function ItineraryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading || !itineraryData) {
    return (
        <div className="min-h-screen bg-[#f0fdfa] flex items-center justify-center text-slate-900">
  ...
  return (
    <div className="bg-[#f0fdfa] text-slate-900 font-sans selection:bg-[#ffc174]/30 min-h-screen relative pb-32">
      <ActionModal
      ...
      {/* Top Navigation Bar */}
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-4 md:px-10 py-4 bg-white/60 backdrop-blur-xl border-b border-teal-100 shadow-sm transition-all duration-300" id="headerNav">
        <div className="flex items-center gap-3">
          <button onClick={() => router.back()} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/60 backdrop-blur-md border border-teal-100 hover:bg-white/10 transition-colors shadow-sm text-[#ffc174] font-semibold text-sm">
          ...
          <Link href="/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/60 backdrop-blur-md border border-teal-100 hover:bg-white/10 transition-colors shadow-sm text-slate-900 font-semibold text-sm">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleDownloadPdf} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/60 backdrop-blur-md border border-teal-100 hover:bg-white/10 transition-colors shadow-sm">
            <Download size={18} className="text-slate-900" />
            <span className="font-semibold text-sm">Download PDF</span>
          </button>
          <button onClick={handleShare} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/60 backdrop-blur-md border border-teal-100 hover:bg-white/10 transition-colors shadow-sm">
      ...
      <section className="relative h-[60vh] md:h-[70vh] w-full overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full px-4 md:px-10 pb-12 md:pb-16 flex flex-col items-start max-w-[1280px] mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 text-slate-900 tracking-tight">{tripParams?.destination || "Your Trip"}</h1>
      ...
      {/* Sticky Tab Bar */}
      <nav className="sticky top-[72px] z-40 px-4 md:px-10 bg-[#f0fdfa]/80 backdrop-blur-md border-y border-teal-50 py-4">
        <div className="max-w-[1280px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2 bg-white/60 backdrop-blur-md border border-teal-100 p-1 rounded-2xl shadow-sm overflow-x-auto w-full md:w-auto">
            <button onClick={() => setActiveTab("timeline")} className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all shrink-0 ${activeTab === 'timeline' ? 'bg-gradient-to-r from-[#d97706] to-[#f59e0b] text-slate-900 shadow-lg shadow-[#d97706]/20 scale-105' : 'text-slate-600 hover:text-slate-900'}`}>Timeline</button>
            <button onClick={() => setActiveTab("overview")} className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all shrink-0 ${activeTab === 'overview' ? 'bg-gradient-to-r from-[#d97706] to-[#f59e0b] text-slate-900 shadow-lg shadow-[#d97706]/20 scale-105' : 'text-slate-600 hover:text-slate-900'}`}>Overview</button>
            <button onClick={() => setActiveTab("budget")} className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all shrink-0 ${activeTab === 'budget' ? 'bg-gradient-to-r from-[#d97706] to-[#f59e0b] text-slate-900 shadow-lg shadow-[#d97706]/20 scale-105' : 'text-slate-600 hover:text-slate-900'}`}>Costs</button>
          </div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-10 mt-16 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-12 min-h-[500px]">
               ...
               {itineraryData.itinerary?.map((day: any) => (
                  <div key={day.day} className="space-y-12">
                     <div className="flex items-center gap-6">
                        ...
                        <div>
                           <h2 className="text-3xl font-bold text-slate-900">Day {day.day} Plan</h2>
                           <p className="text-slate-600 mt-1">Starting your journey at the heart of {tripParams?.destination?.split(',')[0]}.</p>
                        </div>
                     </div>
                        ...
                        {day.activities?.map((activity: any, idx: number) => {
                           ...
                           return (
                              <div key={idx} className="relative group">
                                 ...
                                 {/* Card */}
                                 <div className={`bg-white/60 backdrop-blur-md border border-teal-100 p-6 rounded-2xl ${scheme.glow} transition-all duration-300`}>
                                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                                       <div className="flex items-center gap-3">
                                          ...
                                          <h3 className="text-xl font-bold text-slate-900">{activity.place}</h3>
                                       </div>
                                       <div className="flex items-center gap-4 text-slate-600/70 text-sm font-medium">
                                    ...
                                    {activity.historical_significance && (
                                       <div className="p-4 rounded-xl bg-[#f59e0b]/10 border border-[#f59e0b]/10 flex gap-4">
                                          ...
                                          <div>
                                             ...
                                             <p className="text-sm text-slate-600/90 italic">{activity.historical_significance}</p>
                                          </div>
                                       </div>
                                    )}
                                    {activity.transport_to_place && (
                                       <div className="mt-4 flex gap-2 flex-wrap">
                                          <span className="px-3 py-1 rounded-full bg-white/5 border border-teal-100 text-xs text-slate-600 flex items-center gap-1.5"><Car size={12}/> {activity.transport_to_place}</span>
                                       </div>
                                    )}
                                 </div>
                              </div>
          ...
          {activeTab === "overview" && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
               <div className="bg-white/60 backdrop-blur-md border border-teal-100 p-8 rounded-3xl shadow-xl">
                  <h3 className="text-2xl font-bold text-slate-900 mb-4">Trip Overview</h3>
                  ...
                  <div className="bg-teal-50 border border-teal-200 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                     <div>
                        <h4 className="font-bold text-slate-800 text-lg flex items-center gap-2"><Sparkles className="text-[#0f766e] w-5 h-5"/> Dive Deeper into a Day</h4>
                        <p className="text-slate-600 text-sm mt-1">Want to plan out a specific day with exact local transit, trivia, and AI chatbot guidance?</p>
                     </div>
                     <Link href="/day-plan">
               ...
               <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-white/60 backdrop-blur-md border border-teal-100 p-6 rounded-3xl border-t-4 border-t-[#d97706] shadow-xl">
                     <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><Sparkles className="text-[#d97706]" size={20}/> Hidden Gems</h3>
                  ...
                  <div className="bg-white/60 backdrop-blur-md border border-teal-100 p-6 rounded-3xl border-t-4 border-t-[#d97706] shadow-xl">
                     <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><Utensils className="text-[#d97706]" size={20}/> Food to Try</h3>
                     <ul className="space-y-3">
          ...
          {activeTab === "budget" && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
               {itineraryData?.transit_options && itineraryData.transit_options.length > 0 && (
                  <div className="bg-white/60 backdrop-blur-md border border-teal-100 p-6 rounded-3xl shadow-xl">
                     <div className="border-b border-teal-100 pb-4 mb-6">
                        ...
                        <p className="text-xs text-slate-600 mt-2 opacity-80">* Travel expenses from Source to Destination are separate from Local Budget.</p>
                     </div>
                     <div className="grid sm:grid-cols-3 gap-4">
                        {itineraryData.transit_options.map((opt: any, i: number) => (
                           <div key={i} className="bg-[#f0fdfa]/50 p-4 rounded-xl border border-teal-100 flex flex-col items-center text-center hover:border-[#ffc174]/50 transition-colors">
                              <div className="font-bold text-slate-900 mb-1">{opt.mode}</div>
                              <div className="text-[#ffc174] font-black text-lg mb-1">{opt.estimated_cost}</div>
                              <div className="text-xs text-slate-600 flex items-center gap-1"><Clock size={12}/> {opt.duration}</div>
                           </div>
                        ))}
                     </div>
                  </div>
               ...
               <div className="bg-white/60 backdrop-blur-md border border-teal-100 p-6 md:p-8 rounded-3xl shadow-xl">
                  <h3 className="text-2xl font-bold text-slate-900 mb-1">Local Estimated Costs</h3>
                  <p className="text-sm text-slate-600 mb-8">Breakdown based on standard pricing for {tripParams?.travelers || 2} traveler(s).</p>
                  ...
                  <div className="p-6 bg-[#04b4a2]/10 rounded-2xl flex items-center justify-between border border-[#04b4a2]/20 mb-8">
                     <div>
                        ...
                        <h3 className="text-3xl font-extrabold text-slate-900 mt-1">{itineraryData?.estimated_cost?.total || "N/A"}</h3>
                     </div>
                     <IndianRupee size={48} className="text-[#0f766e]/50" />
                  </div>

                  <div className="space-y-4">
                     <div className="flex items-center justify-between p-4 bg-[#f0fdfa]/40 rounded-xl border border-teal-50">
                        <div className="flex items-center gap-3">
                           <div className="p-2 bg-blue-500/20 text-blue-400 rounded-lg"><Bed size={20} /></div>
                           <span className="font-semibold text-slate-900">Stay & Accommodation</span>
                        </div>
                        <span className="font-bold text-slate-900">{itineraryData?.estimated_cost?.stay || "N/A"}</span>
                     </div>
                     <div className="flex items-center justify-between p-4 bg-[#f0fdfa]/40 rounded-xl border border-teal-50">
                        <div className="flex items-center gap-3">
                           <div className="p-2 bg-[#d97706]/20 text-[#d97706] rounded-lg"><Utensils size={20} /></div>
                           <span className="font-semibold text-slate-900">Food & Dining</span>
                        </div>
                        <span className="font-bold text-slate-900">{itineraryData?.estimated_cost?.food || "N/A"}</span>
                     </div>
                     <div className="flex items-center justify-between p-4 bg-[#f0fdfa]/40 rounded-xl border border-teal-50">
                        <div className="flex items-center gap-3">
                           <div className="p-2 bg-indigo-500/20 text-indigo-400 rounded-lg"><Car size={20} /></div>
                           <span className="font-semibold text-slate-900">Local Transport</span>
                        </div>
                        <span className="font-bold text-slate-900">{itineraryData?.estimated_cost?.transport || "N/A"}</span>
                     </div>
                     <div className="flex items-center justify-between p-4 bg-[#f0fdfa]/40 rounded-xl border border-teal-50">
                        <div className="flex items-center gap-3">
                           <div className="p-2 bg-[#4fdbc8]/20 text-[#0f766e] rounded-lg"><Ticket size={20} /></div>
                           <span className="font-semibold text-slate-900">Entry Fees & Activities</span>
                        </div>
                        <span className="font-bold text-slate-900">{itineraryData?.estimated_cost?.entry_fees || "N/A"}</span>
                     </div>
                  </div>
               </div>
                  ...
                  <div className="bg-[#4fdbc8]/10 border border-[#4fdbc8]/20 p-6 rounded-3xl shadow-xl">
                     ...
                     <ul className="space-y-4">
                        {itineraryData.budget_saving_tips.map((tip: string, i: number) => (
                           <li key={i} className="flex items-start gap-3 text-sm md:text-base text-slate-900">
        ...
        <aside className="lg:col-span-4 lg:sticky lg:top-40 h-fit space-y-6">
          <div className="bg-white/60 backdrop-blur-md rounded-3xl overflow-hidden shadow-2xl border border-teal-100">
            {/* Map Placeholder */}
            ...
            <div className="p-8">
               ...
               <div className="space-y-6 text-sm font-semibold text-slate-600">
                  <div className="flex justify-between items-center border-b border-teal-50 pb-4">
                     ...
                     <span className="text-slate-900 font-bold">{tripParams?.destination?.split(',')[0]}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-teal-50 pb-4">
                     <span>Duration</span>
                     <span className="text-slate-900 font-bold">{tripParams?.days} Days</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-teal-50 pb-4">
                     <span>Style</span>
                  ...
                  <div className="flex justify-between items-center pb-2">
                     <span>Travelers</span>
                     <span className="text-slate-900 font-bold">{tripParams?.travelers || 2} Explorers</span>
                  </div>
               </div>
               {id === 'new' && (
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl z-50">
         ...
         <form onSubmit={handleChatSubmit} className="bg-white/80 backdrop-blur-2xl p-2 rounded-full flex items-center gap-3 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-[#ffc174]/20 transition-transform hover:scale-[1.02] duration-300">
            ...
            <input 
               ...
               className="flex-1 bg-transparent border-none focus:ring-0 text-slate-900 placeholder:text-slate-600/50 text-sm md:text-base font-semibold px-2 outline-none disabled:opacity-50" 
               placeholder="Ask AI to tweak this itinerary..." 
               type="text"
            />
            <button type="submit" disabled={chatLoading} className="w-12 h-12 flex-shrink-0 bg-[#d97706] rounded-full flex items-center justify-center text-slate-900 hover:scale-110 active:scale-90 transition-all shadow-lg shadow-[#d97706]/30 disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed">
