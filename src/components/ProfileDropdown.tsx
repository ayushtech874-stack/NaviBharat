...
export default function ProfileDropdown({ user }: ProfileDropdownProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {/* Dropdown Panel */}
      <div
        className={`absolute right-0 top-full mt-3 w-64 bg-slate-900/95 backdrop-blur-2xl border border-teal-100 rounded-2xl shadow-[0_25px_50px_rgba(0,0,0,0.8)] z-[999] overflow-hidden transition-all duration-300 origin-top-right
          ${open ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}
      >
        {/* User Info Header */}
        <div className="p-4 border-b border-teal-100 bg-gradient-to-r from-amber-600/10 to-transparent">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-bold text-slate-900 truncate">{user?.name || "Traveler"}</p>
        ...
        <div className="p-2">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:text-slate-900 hover:bg-white/5 transition-all group/item text-left"
          >
